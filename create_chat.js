          ...
          messages.map((msg) => {
            ...
            return (
              <div key={msg.id} className={\lex \\}>
                <div 
                  className={\elative max-w-[85%] sm:max-w-[70%] rounded-2xl px-4 py-2 shadow-sm group \\}
                ...
                >
                  ...
                  <div className={\lex items-center gap-2 mt-1 text-[10px] \\}>
                  ...
                  {msg.liked_by && msg.liked_by.length > 0 && (
                    <div className={\bsolute -bottom-2 \ bg-white dark:bg-gray-800 rounded-full shadow-md px-1.5 py-0.5 text-xs border border-gray-100 dark:border-gray-700 flex items-center gap-1 z-10\}>
                      👍 <span className="text-gray-600 dark:text-gray-300 font-medium">{msg.liked_by.length}</span>
                    </div>
                  )}

                  <div className={\bsolute top-2 \ opacity-0 group-hover:opacity-100 flex flex-col items-center gap-1 transition-opacity\}>
                    {isMine && !editingId && (
...
if (!fs.existsSync('C:/Users/User/Desktop/H.Masoud/library-system/src/app/chat')) {
    fs.mkdirSync('C:/Users/User/Desktop/H.Masoud/library-system/src/app/chat', { recursive: true });
}
fs.writeFileSync('C:/Users/User/Desktop/H.Masoud/library-system/src/app/chat/page.tsx', content, 'utf8');
